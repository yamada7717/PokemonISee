...
| 機能                          | 説明                                                                                     |
|-------------------------------|------------------------------------------------------------------------------------------|
| **会員登録・ログイン**     | Sorceryを使用してユーザーの会員登録とログイン機能を実装。                                 |
...
## 自己PR
私は、ポケモン構築記事を読む友人の声をきっかけに、より便利でユーザーフレンドリーな構築記事サイトを作りたいと思い、今回のアプリを開発しました。友人にヒアリングを行い、既存のサイトで感じている不便さや改善点を具体的に聞き出し、それをもとに「ポケモン名や持ち物を同時に検索できる機能」や「構築記事の公開・非公開にする」など、ユーザーにとって役立つ機能を実装しました。

このプロジェクトを通じて、ユーザー視点での要件定義や、フィードバックを活用した機能改善の重要性を学びました。今後も、ユーザーが必要としている機能を的確に把握し、より良いサービスを提供できるエンジニアを目指していきます。
